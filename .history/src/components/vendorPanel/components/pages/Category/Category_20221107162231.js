...
const Category = () => {
  ...
  const [parentCategory, setPC] = useState("");
  ...
  useEffect(() => {
    fetchParentCategory();
  }, [axios, toast, setLoading, setParentC]);
  //=========================================================
  ...
  const addCategory = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        "https://desh-deepak-backend.herokuapp.com/api/v1/admin/category/new",
        { parentCategory },
        auth
      );
  ...
  const deleteCategory = async (id) => {
    try {
      const data = await axios.delete(
        `https://desh-deepak-backend.herokuapp.com/api/v1/admin/category/${id}`
      );
                  ...
                  <tr key={index} style={{ marginTop: "1%" }}>
                    ...
                    <td>
                      <span style={{ display: "flex", gap: "5px" }}>
                        ...
                        <AiFillDelete color="red" cursor="pointer"  onClick={()} />
                      </span>
                    </td>
                  </tr>
                ))}
