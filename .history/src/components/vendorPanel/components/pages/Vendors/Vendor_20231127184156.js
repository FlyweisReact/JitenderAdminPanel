  ...
  const fetchData = async () => {
    try {
      const { data } = await axios.get(
        "https://mr-jitender-backend.vercel.app/api/v1/vender/all",
        {
          headers: {
    ...
    const addVendor = async (e) => {
      ...
      try {
        const { data } = await axios.post(
          "https://mr-jitender-backend.vercel.app/api/v1/vender/register",
  ...
  const statusChange = async (id) => {
    try {
      const { data } = await axios.put(
        `https://mr-jitender-backend.vercel.app/api/v1/vender/status/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
