...
const Product = () => {
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const postData = async (e) => {
      ...
      try { 
        const { data } = await axios.post("http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:5004/api/v1/admin/product/new" , fd)
        console.log(data)
        props.onHide()
        fetchData()
        alert('Product Added')
        ...
        <Modal.Body>
          <Form onSubmit={postData}>
            <Form.Group className="mb-3">
              ...
              <Form.Control type="file" onChange={(e) => setImage(e.target.files)} multiple />
            </Form.Group>
            <Form.Group className="mb-3">
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control type="text"      onChange={(e) => setSize0(e.target.value)} />
            </Form.Group>
            <Form.Group className="mb-3">
              ...
              {categoryP?.categories?.map((i , index) => (
                <option value={i.nam} > {i.name} </option>
              ))}
