...
const Product = () => {
  ...
  const addProduct = async (e) => {
    ...
    const form = {name , description , images , price , category , coloursAvailable , sizesAvailable}
    const auth = { headers  : { Authorization : `Bearer `}}

    try {
      const {data} = await axios.post('https://desh-deepak-backend.herokuapp.com/api/v1/admin/product/new')
