...
const Order = () => {
  ...
  const postHandler = async (e) => {
    e.preventDefault()
    try{
      const { data } = await axios.post(`http://ec2-65-1-248-95.ap-south-1.compute.amazonaws.com:5004/api/v1//user/paymentstatus/${id}`)
      console.log(data)
      fetchData()
    }catch(e)  { 
      console.log(e)
            ...
            <tbody>
              {data?.orders?.map((i, index) => (
                <tr key={index}>
                  ...
                  <td><i className="fa-solid fa-file-pen" onClick={() =} ></i></td>
                </tr>
              ))}
            </tbody>
          </Table>
